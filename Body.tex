...
H.264/Advanced Video Coding (\ac{AVC}) is one of the most commonly used video coding standard. Figure \ref{fig:Hybrid Video Coding} depicts the underlying principle of block-based hybrid video coding used in H.264~\cite{h.264-overview}. The encoding scheme aims to exploit the spatial and temporal redundancies that exist in a video. 

In this coding scheme, the input picture is represented in block-shaped units (16x16 pixels) of associated luma and chroma samples called macroblocks (\ac{MB}s). The basic source-coding algorithm is a hybrid of inter-picture prediction to exploit temporal statistical redundancies and transform coding of the prediction residual to exploit spatial statistical dependencies. The two types of prediction used are:
\begin{description}
	...
	\item[Inter Prediction:] When the framerate is sufficiently high, there is a great amount of similarity between the neighboring frames. It is highly efficient to code the difference between such similar frames than the frames themselves. In inter prediction, block-based motion estimation (\ac{ME}) is used to predict the motion of macroblocks relative to the previously encoded frames.
\end{description}
...
\begin{figure}[h!]
	...
	\caption{Basic coding structure for block-based hybrid video coding~\cite{h.264-overview}.}
	\label{fig:Hybrid Video Coding}
\end{figure}

\section{Bitrate Control} \label{Intro:Bitrate-Control}
The bitrate control module is responsible for controlling the bit-consumption of the encoder to guarantee smooth playback. Bitrate control is not specific to a video coding standard and hence operates independent of any chosen video coding standard. There are various flavors of bitrate control like Constant Bitrate (\ac{CBR}) and Variable Bitrate (\ac{VBR}). In this work, CBR type of bitrate control is considered since it is most commonly used in video conferencing and other real-time streaming applications. 
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.6]{New_dec/RC_blockNew}
...
\begin{itemize}  
	\item \textit{Preprocessing} - Example: blurring.
	\item \textit{Video Coding} - Example: during the rate-distortion optimization (\ac{RDO}), bitrate control.
\end{itemize}
...
The resources available during encoding like computational power is preferentially allocated to ROI in \cite{ROI-resource-allocation}. The H.264 coding standard offers many methods to enhance its compression performance such as variable block size ME, quarter-sample-accurate ME, and multiple reference frames ME~\cite{h.264-overview}. The complexity of H.264 encoder is significantly increased due to the employment of these new methods. The computation need for non-ROI is reduced significantly at the cost of compression efficiency by adaptively adjusting the coding parameters as follows:
\begin{itemize}
	\item Choose only a subset of macroblock partition and prediction modes offered by H.264 standard for non-ROI during rate-distortion optimization to find the best mode. This is similar to the usage of only AMC-based prediction modes for non-ROI proposed in \cite{ROI-rate-control-H264}.
...
\begin{description}
	\item[No B-Frames:] During the low-delay video encoding, tools like the bi-directional prediction (B-frames) are disabled. The usage of B-frames needs buffering of at least one frame. This adds on to the overall latency of the system which is highly undesirable in video conferencing.
	\item[Reduced buffer size:]The tolerable delay in video encoding is a direct measure of VBV size. When the size of VBV is very low (due to low delay), there is less room to accommodate the variation in the bitrate of the encoder. This implies that there can be minimum variation in the frame size across different frames irrespective of the content.
	\item[Increased dropped frames:] Any wrong prediction of QP by the bitrate control module can have a bad impact since there is no additional time available to re-encode the content with a corrected QP. For instance, the over-consumption of bits by a frame which can lead to dropped frames cannot be corrected by re-encoding the frame with a higher QP.
\end{description}

    

The bitrate control module used in this work is a modified version of \cite{JVTF086}. The bitrate control performs frame level bit-allocation based on fullness of the VBV, followed by adapting QP at the macroblock level based on the structural complexity of the macroblock. The functionality of the bitrate control module can be divided into two stages:
\begin{itemize}  
	\item Bit Allocation
	\item QP Prediction
...
The allocated bits for the $nth$ frame is the maximum amount of bits that can be transferred along with the residual bits in the VBV in the duration L ($49\,ms$ in the above example). The maximum acceptable delay in ms (L) is translated to bits ($L_{bits}$) as,
$$ L_{bits} = \frac{L \times bitrate}{1000}.$$
...
\end{cases}
\end{equation}
Where, $avg\_act$ is the average activity across all the macroblocks of a frame. As depicted in the above equation, a positive $dq$ is used when the current macroblock is relatively complex compared to the average frame complexity. This indicates that for relatively complex macroblocks within a frame, a higher QP is used. The purpose of QP adaptation is to account for spatial and temporal masking effect of HVS. Such activity based QP adaptation results in an uneven quality within a frame. The peak signal to noise ratio (\ac{PSNR}) of the simple or static region is higher than that of the regions with high motion (foreground). In practice, the average frame activity of the entire frame is unavailable until the last macroblock of the frame has been encoded. Therefore, previous frame average activity is used as current frame activity since the two adjacent frames in a video are likely to remain similar. 

The activity metric used in \cref{Eq:deltaQP} is a measure of the complexity of the macroblock, hence it can be replaced by similar metrics depicting the complexity of the block. Other metrics like \ac{SATD} (Sum of Absolute Difference in Transform Domain) and cost of the macroblock (J) can be used instead of the activity. In this work, the cost of the macroblock computed during rate-distortion optimization (\cref{Eq:CostCalc}) is used as the complexity metric. 
\begin{equation}
\label{Eq:CostCalc}
J = D + \lambda R,
\end{equation}
where, the distortion D represents the residual error after prediction which is measured as the sum of absolute differences (\ac{SAD}) between the original block and the reconstructed block, and is weighted against the number of bits R associated with the motion information using the Lagrange multiplier $\lambda$. The least cost of all the evaluated modes is considered as the complexity of the block. The cost of the macroblock factors in both the amount of residual information to be encoded after motion compensation and bits used for signaling the mode and the motion vector. This makes it more accurate in terms of reflecting the complexity of the block compared to the activity computed in \cref{Eq:deltaQP}.

\subsection{Bitrate Deviation} \label{sec: Bitrate overview: Bitrate deviation}
The delta QP computed in \cref{Eq:deltaQP} is added to the QP calculated based on the deviation in the bitrate reflected by instantaneous VBV fullness. The buffer fullness corresponds to fullness of the VBV discussed in the context of leaky bucket model in \Cref{Intro:Bitrate-Control}. Any deviation in the bitrate will be reflected in the occupancy of the buffer. For example, a higher level of the buffer indicates over-consumption of bits. The VBV buffer occupancy ($d_0^n$) is calculated only after an entire frame is encoded. In order to account for the deviation in bitrate at the macroblock level, a global deviation factor is computed at the macroblock level. The global deviation is computed based on the deviation in frame level bit-consumption of past frames and the size of the macroblocks encoded in the current frame. The global deviation factor ($D_m^{n'}$) when encoding the $mth$ macroblock of $nth$ frame is given by,
\begin{equation}
\label{Eq:buffer_fullness}
D_m^{n'} = d_0^{n'} + CurFrameBitCount - \frac{B_{alloc} \times m}{M}.
\end{equation}
...
Where, M is the total number of macroblocks in a frame, $B_{alloc}$ is the bits allocated to the frame by bit-allocation module (\cref{Eq:bit-allocation}) and hence remains a constant for the given frame. CurFrameBitCount is the bit-consumption of the current frame until the last encoded macroblock. The term $d_0^{n'}$ in \cref{Eq:buffer_fullness} is the accumulated frame level bit-deviation which is computed similar to the VBV fullness ($d_0^n$). Since the VBV fullness ($d_0^n$) is computed only after fully encoding the frame, the factor $d_0^{n'}$ also remains constant for the given frame. The two terms $d_0^{n'}$ and $d_0^n$ differ with initialization values at the beginning of the encoding \cite{JVTF086}. The frame level deviation factor ($d_0^{n'}$) is additionally subjected to clipping as shown in \cref{Eq:DevClip} after encoding every frame. 
...
\begin{equation}
\label{Eq:mainQP}
Q_m = \frac{D_m^{n'} \times 31}{r} + dq,
\end{equation}
where,
\begin{equation}
r = i \times bitrate/framerate \nonumber.
\end{equation}
...
\section{Sample Video Sequences} \label{sec:sampleInput}
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c| }
		\hline
		\textbf{Name} & \textbf{Resolution} & \textbf{Framerate (Frames/second)} & \textbf{Total Frames} \\
		\hline 
		\textit{Paul} & 640$\times$480 & 30 & 1000 \\ 
		\textit{Chet} & 640$\times$480 & 30 & 490 \\
		\textit{Johnny}\,\cite{JohnnySrc} & 1280$\times$720 & 30 & 300 \\
		\hline
	\end{tabular}
	\caption{Sample video  sequences and their properties.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{\textit{Chet}, frame number 106}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Paul_119.png}
		\caption{\textit{Paul}, frame number 119}
		\label{fig:PaulDefault120}
	\end{subfigure}
	\begin{subfigure}[t]{0.98\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Johnny_35.png}
		\caption{\textit{Johnny}, frame number 35}
		\label{}
	\end{subfigure}
	\caption{Snapshot of sample video sequences (uncompressed).}
	\label{fig:OriginalFrames}
\end{figure}
The three chosen sample video sequences have different spatial and temporal complexities. The sample input videos are chosen to cover most of the typical video conferencing environments. The sample video sequence \textit{Johnny} \cite{JohnnySrc} is a commonly used content for evaluation of video coding algorithms. The sample video sequences \textit{Paul} and \textit{Chet} are chosen from Citrix database. The snapshots of the chosen sample video sequence is shown in Figure \ref{fig:OriginalFrames}. Table \ref{Table:Input-Complexity} lists the relative spatial and temporal complexities across different contents. The table also lists the relative area of the face (ROI) within a frame throughout the given sequence. The relative ROI area ($A_{roi}$) is defined as,
$$A_{roi} = \frac{M_{roi}}{M},$$
where, $M_{roi}$ is the number of ROI macroblocks and M is the total number of macroblocks in a given frame. The area of face region in \textit{Chet} changes significantly within the sequence, other input sequences have almost constant $A_{roi}$ across all the frames.
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c| }
		...
		\hline 
		\textit{Paul} & Medium & Low & 0.05  \\
		\textit{Chet} & Medium & High & 0.04 - 0.4 \\
		\textit{Johnny} & Low & Low & 0.10 \\  
		\hline						
	\end{tabular}
	\caption{The relative spatial and temporal complexity comparison for the sample video sequences in \Cref{Table:ContentNames}.}
	\label{Table:Input-Complexity}
\end{table}

\section{Encoder Configuration} \label {sec:Encoder configuration}     
This work uses the Citrix H.264 video codec for studying different ROI-based encoding approaches. The encoder is configured in low-delay mode suitable for video conferencing and other real-time applications. The bitrate control described in \Cref{chapter:used-bitrate-control-overview} is used to control the output data-rate of the encoder. The encoder is configured to use IPPP mode with key/I-frame used only at the beginning of the sequence followed by only uni-directional P-frames. Due to the low-delay requirements there is no provision to re-encode the frame in case of buffer overflow. The frames are dropped entirely in case of a buffer overflow to maintain a constant low-delay.
\begin{table} [!h]
	\centering
	\begin{tabular}{ |c|c|c| }
		\hline
		\textbf{Name} & \textbf{Bitrate (Kbps)} & \textbf{VBV Size (Bits)} \\
		\hline 
		\textit{Paul} & 250 & 12500 \\ 
		\textit{Chet} & 250 & 12500 \\
		\textit{Johnny} & 350 & 17500 \\
		\hline
	\end{tabular}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{\textit{Chet}, 250\,Kbps}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/PaulDefault250kbps.png}
		\caption{\textit{Paul}, 250\,Kbps}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.98\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Johnny350kbpsDefault.png}
		\caption{\textit{Johnny}, 350\,Kbps}
		\label{}
	\end{subfigure}
	\caption{Result of conventional encoding with corresponding bitrates.}
	\label{fig:ConventionalEncodingFrames}
\end{figure}
The configuration used for encoding each of the sample input sequences listed in the above section is shown in Table \ref{Table:ConfigurationForEncoding}. The bitrates for different input sequences are configured considering the resolution and the complexity of the content. The chosen bitrates are relatively low yielding low visual quality with conventional encoding. This makes it easy to assess the gain in the perceptual quality with ROI-based encoding. Figure \ref{fig:ConventionalEncodingFrames} shows the results of conventional encoding for the configuration shown in Table  \ref{Table:ConfigurationForEncoding}. Due to the usage of low bitrate, blocky artifacts can be noticed in the face regions.
...
\begin{itemize}  
	\item \textit{Quality metrics} - PSNR of ROI and non-ROI parts.
	\item \textit{PSNR and QP maps} - The map of variation of QP and PSNR within a frame.
	\item \textit{Delay plot} - Plots time delay for every frame.
\end{itemize}
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c| }
		\hline
		\textbf{Content} & \textbf{PSNR Avg (dB)} & \textbf{PSNR ROI (dB)} & \textbf{PSNR non-ROI (dB)} \\
		\hline 
		\textit{Paul}, 250\,kbps & 39.15 & 37.72 & 39.22\\ 
		\textit{Chet}, 250\,kbps & 31.35 & 32.91 &  31.24\\
		\textit{Johnny}, 350\,kbps & 37.93 & 36.08 & 38.13\\  
		\hline
	\end{tabular}
...
\begin{itemize}
	\item \textit{Average of frame PSNR} - This is the average of PSNR of all the frames in the video sequence.
	\item \textit{Average MSE-based PSNR} - The PSNR of average MSE of all the frames in the sequence. This is computed by accumulating MSE over entire sequence and then computing PSNR.
\end{itemize}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.98\textwidth}
		...
		\caption{\textit{Johnny}}
		\label{}
	\end{subfigure}
	\caption{Result of conventional encoding - Quantization maps (QP range of 1\,-\,51).}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.98\textwidth}
		...
		\label{}
	\end{subfigure}
	\caption{Result of conventional encoding - PSNR maps (PSNR range of 25\,dB\,-\,45\,dB).}
...
In constant bitrate control, the size of an encoded frame decides the buffering delay of each frame. Therefore, any change in bit-consumption pattern at the frame level gets reflected in the buffering delay of the corresponding frames. The delay due to buffering of each frame in the sequence is plotted to analyze the bit-consumption behavior. Figure \ref{fig:ConventioanlEncodingDelay} is the delay plot of the sample video sequences encoded with the configuration shown in Table \ref{Table:ConfigurationForEncoding}. Every point in the delay plot specifies the time taken by the corresponding frame (marked on the x-axis) to reach the decoder assuming zero transmission delay. It can be noticed that the delay is almost constant for \textit{Johnny} and \textit{Paul}. However, there is a slight variation for \textit{Chet} due to high temporal complexity.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Chet250kbpsDefaultDelayNew.png}
		\caption{\textit{Chet}}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/PaulDefault250kbpsDelayNew.png}
		\caption{\textit{Paul}}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Johnny350kbpsDefaultDelayNew.png}
		\caption{\textit{Johnny}}
		\label{}
	\end{subfigure}
	\caption{Result of conventional encoding - Delay plots.}
...
%Face Detection
%
\chapter{Face Detection and Tracking} \label{chapter:FaceDetection}
\thispagestyle{empty}% no page number in chapter title page
...
The face detection module is independent of the encoder and ROI-based encoding schemes discussed in this work. The face detection module uses input video stream to the encoder and marks the ROI at the macroblock level. Figure \ref{fig:PaulDefault120FaceMap} shows the face map generated for the frame shown in Figure \ref{fig:PaulDefault120}. Each byte value in the face-map generated by the face detection module represents a macroblock scanned in raster scan order. The region in white is considered as ROI. This information is used by the bitrate control module of the encoder for ROI-based encoding.
...
\begin{figure}[h!]
	...
	\caption{Face detection output for \textit{Paul} and the corresponding face map. The white and black regions represent ROI and non-ROI respectively.}
	\label{fig:PaulDefault120FaceMap}
\end{figure}

\section{Viola Jones Face Detection}
Many open-source solutions like OpenCV offers a ready to use solution that can be integrated with the codec library. In this work, OpenCV implementation of Viola-Jones AdaBoost face detection algorithm~\cite{Viola01rapidobject} is used. It has a large set of trained classifiers considering many types of faces and viewing angles. A standard frontal face Haar-classifier is used to detect faces in all the sample video sequences.

\subsection{Performance Benchmark}
In this work, face is detected independently in every frame of the video stream. This is used to achieve high accuracy face detection to evaluate ROI-based encoding techniques. This setup is not the most optimal solution to be used in a real-time system. Face detection is a computation intensive task and performing this task on every frame of the video stream demands huge computation power. \Cref{Table:FaceDetectionBenchMark} gives the measured performance of OpenCV based face detection on \textit{Intel(R) Xeon(R) CPU E3-1225 V2 @ 3.20\,Ghz}. The numbers mentioned in \Cref{Table:FaceDetectionBenchMark} also account for input raw file format conversion in addition to face detection. It is clear that the face detection module complexity increases almost linearly with increase in resolution. For a resolution of 1280$\times$720, the processing is not real-time (assuming 30\,fps video stream) with the specified computation resource. The following section proposes an optimization scheme to detect the face in real-time.
\begin{table} [h!]
	\centering
	\begin{tabular}{ |c|c|c| }
		\hline
		\textbf{Name} & \textbf{Resolution} & \textbf{Frames/second} \\
		\hline 
		\textit{Paul} & 640$\times$480 & 44.26 \\ 
		\textit{Vidyo4}\,\cite{JohnnySrc} & 1280$\times$720 & 18.89 \\
		\hline
	\end{tabular}
	\caption{OpenCV face detection performance benchmark on \textit{Intel(R) Xeon(R) CPU E3-1225 V2 @ 3.20\,Ghz}.}
	\label{Table:FaceDetectionBenchMark}
\end{table}

\section{Optimization Using Motion Vectors}
As mentioned above, it is not an optimal solution to invoke face detection module for every frame of the input video stream. Usually, face tracking techniques are used to track the face once it is detected. Some of the conventional approaches for tracking are mean-shift and continuous adaptive mean shift~(\ac{CAMSHIFT}) \cite{CAM-shift}. However, these techniques also demand a considerable amount of computation resource and requires additional effort to implement face tracking in addition to face detection.

\subsection{Regular Interval Face Detection}
An alternative approach is to detect the face at regular intervals in every $nth$ frame instead of every frame of the video. Due to the high framerate of the input video stream, the movement of the face region is limited between the consecutive frames. The intermediate frames consider the previously detected face region as the face. In other words, the approach is to use the temporally subsampled video stream with sampling interval $n$ as input to the face detection module. However, this approach of temporally subsampled face detection leads to inaccuracy which increases with the sampling interval between two frames in which the face is detected. The inaccuracy also increases with rapid movements in the face region. 

\Cref{fig:FDnFrameWorstError} shows a snapshot of the sample input content \textit{Vidyo4} \cite{JohnnySrc} with 30 frames per second with face detection at every frame and at regular intervals (temporal sub-sampling). The green bounding box represents the output with face detection on every frame ($n=1$) and the red bounding box represents face detection performed on every 15th frame ($n=15$). It is clear that there is a large inaccuracy with temporally subsampled face detection. For sampling interval $n=15$, majority of the face region lies outside the bounding box. This can degrade the performance of ROI-based encoding since the region most important to the perceptual quality is considered as non-ROI. The error due to temporal subsampling is measured as the euclidean distance (pixels) between the top-left corner pixels of the two bounding boxes. The error is always computed with respect to the green bounding box ($n=1$). The error with different sampling intervals for face detection is tabulated in \Cref{Table:FaceDetectionError}.

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{FaceTracking/everyNframe/vidyo4_180_worstcaseerror.png}
	\caption{Snapshot of \textit{Vidyo4} with face detection at regular intervals (temporal sub-sampling). Green and red bounding boxes represent the output of face detection performed on every frame and every 15th frame respectively.}
...
\begin{table}[h!]
	...
	\begin{tabularx}{0.9\linewidth}{ |Y|Y|Y| }
		...
		\hline
	\end{tabularx}
	\caption{Inaccuracy due to face detection at regular intervals ($n$) for the sample content \textit{Vidyo4}. The error is computed with respect to the output with face detection performed on every frame ($n=1$).}
...
\begin{itemize}
	\item The face detection module is invoked even when the face region is static. This is a wastage of precious computation resource.
	\item The error during sharp movements in face region is very high (\Cref{Table:FaceDetectionError}).
\end{itemize} 

In order to address the above disadvantages, this work proposes an alternative approach in which motion vector information computed during motion estimation stage of video encoding is reused to detect the movement of face to invoke face detection instead of detecting the face in every $nth$ frame.

\subsection{Face Movement and Motion Vector}
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.9]{FaceTracking/MotionVSMotionVectorVidyo4New.png}
	\caption{Comparison of the movement of the face region between consecutive frames (blue) and the corresponding average magnitude of motion vectors in the face region (red).}
	\label{fig:MotionVSMotionVector}
\end{figure}
Any movement in the face region is reflected in the motion vectors of macroblocks in the corresponding regions. The magnitude of these motion vectors is directly proportional to the magnitude of movement in the face region. \Cref{fig:MotionVSMotionVector} shows the predictability of sharp movements in the face region with average magnitude of motion vectors of the face region. The series in blue represents the difference between top-left corner pixels of the bounding boxes of detected face in a given frame and its previous frame. The face detection is performed on every frame of the video stream ($n=1$). The series in red represents the average magnitude of motion vectors in the face region of any given frame predicted from its previous frame. It is clear that all sharp movements in the face region have correspondingly high magnitude motion vectors. This shows that sharp movements in the face region are easily predictable from the motion vectors of the corresponding regions.
...
\begin{enumerate}
	...
	\item For every frame, compute the average magnitude of motion vectors of all the macroblocks belonging to the face region. If this value is greater than a predetermined threshold ($T_H$), perform face detection on the current frame.
	\item It is possible that face region can slowly drift with the average motion vector less than $T_H$ on every frame. To correct the accumulated error due to such gradual movement of the face region, face detection is performed after a large number of frames ($N$) even if no motion is detected. Since the movement of the face region is very slow in this case, a large sampling interval $N$ can be used such that $N >> n$ . 
\end{enumerate}
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{FaceTracking/motion_vector/vidyo4_180_worstcaseerror.png}
	\caption{Face detection at regular intervals as shown in Figure \ref{fig:FDnFrameWorstError} along with motion vector based variable interval face detection (blue bounding box).}
	\label{fig:FD_all}
\end{figure}

The technique described above will avoid the wastage of computation resource by avoiding face detection or tracking when the face region is highly static. This will also reduce the error due to face detection with temporal subsampling since face detection is performed immediately after considerable movement of the face region is detected. \Cref{fig:FD_all} shows the same frame shown in \Cref{fig:FDnFrameWorstError} with motion vector based variable interval face detection (blue bounding box). It is clear that the inaccuracy in face detection is much lower compared to the approach where face is detected in every 15th frame ($n=15$). 

%Table showing improvement with face detection based on motion vectors
\begin{table} [h!]
	\centering
	\begin{tabularx}{0.99\linewidth}{ |Y|Y|Y|Y| }
		\hline
		...
		\hline
	\end{tabularx}
	\caption{Performance of regular interval and motion vector based variable interval face detection for the sample content \textit{Vidyo4} with 300 frames.}
...
\begin{table} [h!]
	...
	\begin{tabularx}{0.9\linewidth}{ |Y|Y| }
		\hline
		\textbf{Face detection interval} & \textbf{Average processing time for face detection (seconds)} \\
		\hline 
...
\begin{itemize}
	...
	\item This approach is based on the reuse of motion vectors computed during video encoding. Therefore, it has very low computation complexity.
\end{itemize}
\subsection*{Disadvantages}
\begin{itemize}
	\item \textit{Unpredictable load} - In motion vector based variable interval face detection, the number of face detection performed for a given sequence depends on the magnitude of motion in the face region. This varies across different contents. Therefore, the processing load due to this approach is not predictable.
	\item \textit{Higher peak load} - Motion vector based variable interval face detection can trigger too many face detection in a small interval due to very high motion in the face region.
\end{itemize}
%\begin{table} [h!]
%	\centering
...
%		& & & \\ 
%		\hline
%		\multirow{2}{*}{\parbox{0.3\linewidth}{Improved QP Offset based ROI encoding}}  & \textit{Paul}, 250kbps & 38.29 & 40.10\\ 
%		& \textit{Johnny} 350kbps & 37.50 & 37.37\\ 
%		\hline
%		\multirow{2}{*}{\parbox{0.3\linewidth}{ROI-based Bit-allocation}} & \textit{Paul}, 250kbps & 38.22 & 40.95\\ 
%		& \textit{Johnny} 350kbps & 36.94 & 38.57\\  
%		\hline 
%	\end{tabular}
...
\begin{itemize}
	...
	\item The rate control module obtains feedback from the encoder regarding bit-consumption at the macroblock level (\Cref{chapter:used-bitrate-control-overview}). This feedback signals over-consumption of bits by ROI macroblocks resulting in a higher deviation factor ($D_m^{n'}$).
...
\begin{itemize}
	\item \textit{Tuning QP offset} - Understanding the effect of using different QP offsets.
	\item \textit{Area of ROI} - The relative area of ROI and non-ROI parts in a frame.
	\item \textit{Bi-direction QP offset} - A positive QP offset is used for non-ROI blocks along with the negative QP offset for ROI.
\end{itemize}
...
\begin{figure}[h!]
	...
	\end{subfigure}
	\caption{Snapshots of \textit{Chet} encoded with QP offset for ROI (Left) and the corresponding PSNR maps with a range of 25\,dB\,-\,45\,dB (Right) for different QP offsets.}
...
The snapshot of same sample input encoded with QP offset of $-8$ shown in Figure \ref{fig:QP offset compare 8} has a sharper face region compared to QP offset of $-4$ shown in Figure \ref{fig:QP Offset image comparison 2}. This is expected since the face region is coded with a lower QP due to a larger negative QP offset. The blockiness in the non-ROI, specifically around the arm region has increased. The increase in sharpness in the face region masks this blockiness to some extent. However, with further increase in the magnitude of QP offset the blockiness in the background becomes more prominent. The image in Figure \ref{fig:QP offset compare 12} is encoded with a QP offset of $-12$. At this point, the increase in sharpness in the face region is masked by extreme blockiness in non-ROI, specifically around the arm region. The corresponding PSNR plots show extreme low PSNR in the non-ROI macroblocks with increasing QP offsets. Therefore, a very high QP offset can have an adverse effect on the perceptual quality.
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c| }
		...
		\textbf{ROI QP Offset} & \textbf{QP Swing Restriction = on} & \textbf{QP Swing Restriction = off} \\
		\hline 
		0 & 16 & 12  \\
		...
		\hline						
	\end{tabular}
	\caption{Number of dropped frames with different QP offsets for ROI for the sample sequence \textit{Chet} with 490 frames.}
...
\begin{figure}[h!]
	...
	%	\centering
		...
		\label{}
	\end{subfigure}
	\caption{Snapshots and the corresponding face maps of the sample content \textit{Chet} (uncompressed), (a) frame number 106 with $A_{roi} = 0.067$ and (b) frame number 446 with $A_{roi} = 0.35$.}
	\label{fig:variable-face-size}
\end{figure}
	
However, in a video conferencing scenario, based on the focal length of the camera and distance of the participant from the camera, area of the face in the video frame can change significantly. The number of ROI macroblocks can also change within a given input video sequence. Figure \ref{fig:variable-face-size} shows the variation in the size of ROI within a video sequence. This demands continuous adaptation of the QP offset to avoid visual artifacts due to the usage of wrong QP offset. For instance, when the area of ROI greater than the area of non-ROI ($A_{roi} > 0.5$), usage of a higher QP offset will lead to huge over-consumption of bits by ROI due to an increased number of ROI macroblocks causing severe degradation in the quality of non-ROI macroblocks. In order to avoid severe degradation of the background, the magnitude of the QP offset should be inversely proportional to the ratio of the number of ROI blocks to non-ROI blocks.

The algorithm implemented to adapt QP offset uses the relative area of ROI. The QP offset is calculated using the linear relationship described in \cref{Eq:QP_offset}. This is a heuristic approximation which was found to yield the best perceptual quality across many different video contents. The scaled QP offset is then clipped to a value of $-6$ to avoid large QP offsets which can lead to side-effects discussed in the previous section.
\begin{equation}
\label{Eq:QP_offset}
\begin{aligned}
dq_{roi'} = -round\Big(\frac{M}{M_{roi} \times 3}\Big), \\
dq_{roi} = clip(dq_{roi'}, -1 , -6),
\end{aligned}	
\end{equation}
where, $dq_{roi}$ is the offset used for ROI blocks, $M$ is the total number of macroblocks in the frame and $M_{roi}$ is the total number of macroblocks marked as ROI. The negative sign in the equation implies that the calculated offset is negative, which results in QP lower than non-ROI blocks. It is also evident from \cref{Eq:QP_offset} that a minimum QP offset ($dq_{roi} = -1$) is used when ROI area is more than two-third of the whole frame. The magnitude of QP offset increases linearly with subsequent decrease in the area of ROI.
...
The study in \cite{HighQualityROICodingForVideoConferencing} suggests that for the frame level bit-count to be constant, the average QP of the frame must remain unchanged before and after adding the offsets. This is an observation made after multiple experiments. The QP offset for non-ROI macroblocks used to compensate the negative QP offset used by ROI is given by,
\begin{equation}
\label{Eq:QP_offset_double}
dq_{nroi} = \frac{M_{roi} \times dq_{roi}}{M - M_{roi}},
\end{equation}
where, $dq_{nroi}$ is a positive QP offset used for non-ROI blocks when negative QP offset of $dq_{roi}$ is used for ROI blocks (\cref{Eq:QP_offset}). 

\subsubsection*{Inference}

The result of QP offset based ROI encoding is discussed in \Cref{sec:ROI-QP-Offset-Results}. It should be noted that the term \lq{}QP offset based ROI encoding\rq{} or \lq{}bi-direction QP offset approach\rq{} refers to the QP offset scheme with all the improvements discussed in the previous \Cref{sec:Tuning QP Offset,sec:QP offset area of ROI,sec:bi-direction QP offset}. 
...
\begin{itemize}
	\item \textit{Relative Bit-consumption Prediction} - Predict the relative bit-consumption of ROI and non-ROI ($B_R$) in conventional encoding.
	\item \textit{ROI and non-ROI Bit Allocation} - Split the allocated frame level bits ($B_{alloc}$) between ROI and non-ROI parts according to $B_R$ with an additional bias for ROI parts.
...
\begin{itemize}
	\item The cost of a macroblock reflects the complexity of the macroblock. A macroblock with a higher cost consumes more bits (\cref{Eq:Relative bit consumption}).
	\item The RDO cost is also used in the bitrate control for delta QP prediction as discussed in \Cref{used QP prediction overview}. The delta QP computation (\cref{Eq:deltaQP}) allocates a higher QP to the macroblock with a higher cost for spatial and temporal masking considering HVS. This will marginally reduce the bit-consumption of the macroblock.
\end{itemize}
...
\begin{figure}[h!]
	...
	\caption{Cost vs bits plot for all the macroblocks of multiple video sequences encoded with constant QP ($QP = 35$).}
...
J = 18.2 \times b + 693.
\end{equation}
The above equation is obtained by performing linear regression on the entire data-set shown in Figure \ref{fig:CostvsBits_ConstQP_Full}. It has a corresponding $R^2$ value of 0.2366. The $R^2$ value (also displayed on the plots) is a measure of correlation between actual data and the predicted data using the linear relationship. A value of $R^2 = 1$ indicates perfect correlation. The $R^2$ value in this case is low to form an efficient prediction model. The correlation between macroblock cost and its bit-consumption is increased further by considering the prediction mode of the macroblock.

\subsubsection*{Cost-Bits Model - Prediction Modes}

The data used in Figure \ref{fig:CostvsBits_ConstQP_Full} includes data for macroblocks of all types of prediction modes like skip, intra and inter prediction. It was observed that the predictability of bit-consumption using RDO cost was heavily dependent on the prediction mode used for encoding the macroblock. 

The analysis of the macroblock cost and its corresponding bit-consumption specific to a prediction mode is shown in Figure \ref{fig: Cost vs bits mode}. The plots in Figure \ref{fig: Cost vs bits mode} are generated using the same data as shown in Figure \ref{fig:CostvsBits_ConstQP_Full}. Different plots are generated for skip, inter and intra prediction modes. It can be noticed that the correlation between the macroblock cost and its bit-consumption is maximum for intra prediction ($R^2 = 0.8757$) followed by inter prediction ($R^2 = 0.2995$). However, the bit-consumption pattern was found to be almost independent of the cost for skip-mode macroblocks ($R^2 = 0.0412$). This is because the skip-mode decision happens in a later stage of encoding when there are no transform coefficients generated for a macroblock encoded with intra or inter prediction. The cost associated with the skip macroblock corresponds to the prediction mode chosen before the macroblock was decided to be encoded as skip macroblock.
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c| }
		\hline
		\textbf{Prediction Mode} & \textbf{Occurrence Probability } & \textbf{$R^2$ value for Cost-bits model} \\
		\hline 
		Intra & 0.23 & 0.816  \\
		Inter & 0.28 & 0.428 \\
		Skip &  0.69 & 0.006 \\ 
		\hline					
	\end{tabular}
	\caption{The probability of prediction modes in constant QP ($QP = 35$) encoded sequences and its corresponding $R^2$ value.}
...
\begin{figure} [h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{CostVsBits/new/CostvsBits_ConstQP_global_inter.png}
		\caption{Inter Prediction mode}
		\label{fig: Cost vs bits mode inter}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{CostVsBits/new/CostvsBits_ConstQP_global_intra.png}
		\caption{Intra Prediction mode}
		\label{fig: Cost vs bits mode intra}
	\end{subfigure}
	\begin{subfigure}[t]{\textwidth}
		\centering
		\includegraphics[width=0.49\textwidth]{CostVsBits/new/CostvsBits_ConstQP_global_skip.png}
		\caption{Skip mode}
		\label{fig: Cost vs bits mode skip}
	\end{subfigure}
	\caption{Cost vs bits plots for all the macroblocks of multiple video sequences encoded with constant QP ($QP = 35$) specific to the mode of encoding. (a) inter prediction, (b) intra prediction, (c) skip mode.}
	\label{fig: Cost vs bits mode}
\end{figure}

The data in Table \ref{Table:Mode Specific data} shows the probability of choosing a given prediction mode. It is clear that skip mode is the most commonly used prediction mode for the encoder configuration discussed in \Cref{sec:Encoder configuration}. This is due to the low target bitrate and mostly static background in video conferencing sample contents. Therefore, it is very important to have good bits vs cost prediction model for skip macroblocks to compute $B_R$ with a reasonable accuracy.

Fortunately, the extremely low correlation between macroblock cost and its bit-consumption for skip mode can be easily handled by assuming a constant number of bits for skip mode and ignoring its cost. The average amount of bits consumed by the skip macroblocks over the entire data-set was found to be 0.458\,bits. This is extremely low bit-consumption compared to the intra and inter mode macroblocks. Therefore, a constant bit-estimate (average bits) is used in \cref{Eq:Relative bit consumption} to compute the relative bit-consumption of ROI and non-ROI based on the number of skip macroblocks in each of these regions. The $R^2$ value of the prediction model is improved considerably by this assumption for the skip macroblocks.
...
\begin{itemize}
	...
	\item Compute the relative complexities of the ROI and non-ROI parts by accumulating costs of macroblocks in these regions.
...
As mentioned earlier, the ROI-based bit-allocation is independent of the procedure involved in determining the relative bit-consumption factor ($B_R$). The approach presented in \Cref{sec:Relative Bit-consumption Prediction} needs computation of the relative complexities of ROI and non-ROI. A simpler approach is to assume that the frame is uniformly complex and splitting the allocated frame level bits ($B_{alloc}$) uniformly between ROI and non-ROI with a bias factor specified in \cref{Eq:ROI allocated bits}. Therefore, if the content information is not considered for simplicity, the bits allocated for ROI and non-ROI is computed using \cref{Eq:ROI allocated bits} with,
...
The bitrate control module studied in \Cref{chapter:used-bitrate-control-overview} computes the deviation factor ($D_m^{n'}$) based on the accumulated error in frame level bit-consumption after encoding every frame as given by \cref{Eq:Frame level bit error accumulation}. In steady-state, the deviation factor ($D_m^{n'}$) takes a value such that the error in frame level bit-consumption is minimum. 
...
\begin{itemize}
	...
	\item A single instance of macroblock level bitrate error is maintained (\cref{Eq:Frame level bit error accumulation}). This ties the two instances of bitrate control together, so that frame level bit-error is minimized. The error in allocation and consumption of bits in ROI and non-ROI is compensated throughout the frame resulting in less overall frame level error.
	\item The VBV compliance works with frame level bit-allocation and therefore remains unchanged. There will be no increase in the number of dropped frames as long as the frame level deviation in bit-consumption is kept under check.
\end{itemize} 
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c|c| }
		\hline
		& & \multicolumn{3}{c|}{\textbf{PSNR\,(dB)}}\\ \cline{3-5}
		\textbf{Methodology} & \textbf{Content} & \textbf{Frame} & \textbf{ROI} & \textbf{non-ROI}\\
		\hline 
		\multirow{3}{*}{\parbox{0.3\linewidth}{Conventional Encoding}} & \textit{Paul}, 250\,kbps & 39.15 & 37.72 & 39.22\\ 
							  & \textit{Johnny}, 350\,kbps & 37.93 & 36.08 & 38.13\\ 
							  & \textit{Chet}, 250\,kbps & 31.35 & 32.91 & 31.24\\ 
		\hline
		\multirow{3}{*}{\parbox{0.3\linewidth}{Improved QP Offset based ROI encoding}}  & \textit{Paul}, 250\,kbps & 38.29 & 40.10 & 38.24\\ 
											   & \textit{Johnny}, 350\,kbps & 37.50 & 37.37 & 37.52\\ 
											   &  \textit{Chet}, 250\,kbps & 31.08 & 34.89 & 30.89\\ 
		\hline
		\multirow{3}{*}{\parbox{0.3\linewidth}{ROI-based Bit-allocation}} & \textit{Paul}, 250\,kbps & 38.22 & 40.95 & 38.15\\ 
								 & \textit{Johnny}, 350\,kbps & 36.94 & 38.57& 36.85\\  
								 &  \textit{Chet}, 250\,kbps & 30.55 & 36.17& 30.30\\
		\hline 
	\end{tabular}
	\caption{PSNR comparison between conventional encoding and different approaches of ROI-based encoding.}
	\label{Table:AllPSNR1}
\end{table}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Chet106_250kbpsDefault.png}
		\caption{Conventional encoding}
		\label{fig:ChetImageComp-conventional}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Qp_offset/Chet106_250kbpsQpOffset.png}
		\caption{ROI QP offset}
		\label{fig:ChetImageComp-Qpoffsets}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/BufferControl/Chet106_250kbpsBuffer.png}
		\caption{ROI-based bit-allocation}
		\label{fig:ChetImageComp-ROiBitrate}
	\end{subfigure}
%For Chet over
	\caption{Comparison of \textit{Chet} encoded at 250\,Kbps with conventional encoding and different ROI-based encoding approaches. The increase in the sharpness of the face region is clearly noticeable in ROI-based encoding approaches.}
...
\begin{subfigure}[t]{0.49\textwidth}
	...
	\label{}
\end{subfigure}
	\caption{Comparison of \textit{Paul} encoded at 250\,Kbps with conventional encoding and different ROI-based encoding approaches. The increase in the sharpness of the face region is noticeable in ROI-based encoding approaches.}
...
\begin{subfigure}[t]{0.7\textwidth}
	...
	\label{}
\end{subfigure}
	\caption{Comparison of \textit{Johnny} encoded at 350\,Kbps with conventional encoding and different ROI-based encoding approaches. The increase in the sharpness of the face region is noticeable in ROI-based encoding approaches.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\label{}
	\end{subfigure}
	\caption{Comparison of PSNR maps (PSNR range of 25\,dB\,-\,45\,dB) of sample contents (a,\,b,\,c) \textit{Chet}, (d,\,e,\,f) \textit{Paul} and (g,\,h,\,i) \textit{Johnny}  for conventional and ROI-based encoding approaches. (a,d,g) Conventional encoding, (b,e,h) ROI QP Offset, (c,f,i) ROI-based bit-allocation.}
...
\begin{figure}[h!]
	...
	\end{subfigure}
	\caption{Comparison of quantization maps (QP range of 1\,-\,51) of sample contents (a,\,b,\,c) \textit{Chet}, (d,\,e,\,f) \textit{Paul} and (g,\,h,\,i) \textit{Johnny}  for conventional and ROI-based encoding approaches. (a,d,g) Conventional encoding, (b,e,h) ROI QP Offset, (c,f,i) ROI-based bit-allocation.}
...
%	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Qp_offset/Chet250kbpsQpOffsetDelayNew.png}
		\caption{\textit{Chet} - ROI QP Offset}
		\label{fig:AllDelayComparison-offset-Chet}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/BufferControl/Chet250kbpsBufferDelayNew.png}
		\caption{\textit{Chet} - ROI-based bit-allocation}
...
%	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Qp_offset/Paul250kbpsQpOffsetDelayNew.png}
		\caption{\textit{Paul} - ROI QP Offset}
		\label{fig:AllDelayComparison-offset-Paul}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/BufferControl/Paul250kbpsBufferDelayNew.png}
		\caption{\textit{Paul} - ROI-based bit-allocation}
...
%	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Qp_offset/johnny350kbpsQpOffsetDelayNew.png}
		\caption{\textit{Johnny} - ROI QP Offset}
		\label{fig:AllDelayComparison-offset-Johnny}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/BufferControl/Johnny350kbpsBufferDelayNew.png}
		\caption{\textit{Johnny} - ROI-based bit-allocation}
		\label{fig:AllDelayComparison-ROI-bitrate-Johnny}
	\end{subfigure}
	\caption{Comparison of Delay plots for the sample sequences for different approaches of ROI-based encoding (Green) and conventional encoding (Purple). The plots show that ROI-based encoding does not alter the delay behavior with respect to conventional encoding in most cases.}
...
%			\label{}
%		\end{subfigure}
%		\caption{Result of ROI-based bit allocation encoding for the encoding configuration discussed in section \ref{sec:Encoder configuration} (a)\textit{Chet}, (b)\textit{Paul} ,(c)\textit{Johnny}.}
%		\label{fig:ConventionalEncodingFrames}
%	\end{figure}
%	%Quant Maps
%	\begin{figure}[h!]
...
%			\label{}
%		\end{subfigure}
%		\caption{Result of ROI-based bit allocation encoding (a)\textit{Chet}, (b)\textit{Paul} ,(c)\textit{Johnny}.}
%		\label{fig:ConventionalEncodingFrames}
%	\end{figure}
%	%Psnr maos
%	\begin{figure}[h!]
...
\subsection*{Disadvantages}
\begin{itemize}
	\item The QP offsets chosen does not take into account any of the input content characteristics. Same QP offset is used for all the contents for a given ratio of ROI to non-ROI area.	
	\item Since buffer conditions are not considered at the macroblock level, there is an increased possibility of dropped frames.
	...
	\item The deviation control mechanism described in \Cref{sec: Bitrate overview: Bitrate deviation} is not in sync with the QP offsets. The deviation control mechanism will increase the QP even for ROI blocks when there is an increased deviation due to over-consumption of bits by ROI macroblocks.
\end{itemize}


\subsection{ROI-based Bit-allocation - Results}
The ROI-based bit-allocation scheme for ROI encoding considers the input content characteristics to perform bit-allocation for ROI blocks. Therefore, this approach is applicable to generic contents and hence can be expected to be more robust across multiple contents. The result of ROI encoding using ROI-based bit-allocation is shown in \Cref{fig:ChetImageComp,fig:PaulImageComp,fig:JohnnyImageComp}. For the sample input \textit{Chet}, there is a clear improvement in the sharpness of the face region with ROI-based bit-allocation (\Cref{fig:ChetImageComp-ROiBitrate}) over conventional encoding (\Cref{fig:ChetImageComp-conventional}) and even ROI encoding using QP offsets (\Cref{fig:ChetImageComp-Qpoffsets}). The degradation of non-ROI region is not noticeable which improves the overall perceptual quality by a large extent. Similar trend is observed for other sample input sequences as well. 

The overall difference in the image quality is more noticeable in the decreasing order for the sample input sequences \textit{Chet}, \textit{Johnny} and \textit{Paul}. This observation can be explained by considering the overall PSNR values. The PSNR values increases in the same order with \textit{Chet} and \textit{Paul} having minimum and maximum PSNR values respectively. The reason for this difference in PSNR between \textit{Chet} and \textit{Paul} is the difference in temporal complexity. Even though the resolution and operating bitrates are the same for both, \textit{Chet} has very high temporal complexity resulting in very low PSNR value. This observation shows that the gain in perceptual quality is more significant at lower overall PSNR.

\begin{description}
	\item[PSNR Value:] The PSNR values for all the sample video sequences with ROI-based bit-allocation is tabulated in \Cref{Table:AllPSNR1}. Similar to observations made with QP offset based ROI encoding, there is a huge improvement in PSNR of the ROI and a small drop in non-ROI PSNR. It can be noticed that PSNR of ROI is higher with ROI-based bit-allocation compared to QP offsets based ROI-encoding. The average improvement in PSNR of ROI is 2.9\,dB with an average drop in non-ROI PSNR of 0.9\,dB across all the chosen sample contents. This shows that ROI-based bit-allocation scheme results in more aggressive movement of bits from non-ROI to ROI parts. This is possible since the input content characteristics are considered in the algorithm which allows it to exploit the static background to allocate maximum amount of bits to complex ROI parts. This is not possible in ROI encoding with QP offset because the approach needs to be conservative due to the lack of information about the content characteristics. The adverse effects of using large magnitude QP offset is discussed previously in \Cref{sec:Tuning QP Offset}.
	 
	\item[PSNR and Quantization Maps:] The comparison of PSNR and Quantization maps with conventional encoding and improved QP offset based ROI-encoding is shown in \Cref{fig:AllPsnrComparison} and \Cref{fig:AllQuantizationComparison} respectively.
	The usage of a lower quantization parameter in the face regions compared to conventional encoding is clearly visible. The quantizations maps corresponding to ROI-based bit-allocation shows more prominent distinction between ROI and non-ROI parts compared to QP offset based approach. This is achieved by allocating bits to ROI and non-ROI parts independently. Unlike QP offset ROI encoding, the deviation control mechanism does not conflict with the higher bits allocated to ROI. This leads to clear distinction between ROI and non-ROI QP. The increase in non-ROI QP is also more uniform with this scheme as shown in \Cref{fig:AllQuantizationComparison-ROI-bitrate-chet,fig:AllQuantizationComparison-ROI-bitrate-Paul,fig:AllQuantizationComparison-ROI-bitrate-Johnny} as compared to QP offset based approach shown in \Cref{fig:AllQuantizationComparison-offset-chet,fig:AllQuantizationComparison-offset-Paul,fig:AllQuantizationComparison-offset-Johnny}.
	 
	The PSNR maps shown in \Cref{fig:AllPsnrComparison} depicts better quality in the face regions compared to conventional and QP offset based ROI encoding. This is due to the aggressive movement of bits from non-ROI parts to ROI parts.
	\item[Delay Plots:] The delay plots for ROI-based bit-allocation are shown in \Cref{fig:AllDelayComparison}. For each of the sample video sequences, the delay curve follows the curve for conventional encoding. However, this is one aspect in which ROI-based bit-allocation scheme performs inferior compared to QP offset based ROI encoding. There is a considerable deviation in the delay curve for \textit{Chet} and \textit{Paul} with ROI-based bit-allocation (\cref{fig:AllDelayComparison-ROI-bitrate-Paul,fig:AllDelayComparison-ROI-bitrate-Chet}) as compared to QP offset based ROI encoding (\cref{fig:AllDelayComparison-offset-Paul,fig:AllDelayComparison-offset-Chet}). There is also an increase in the number of dropped frames which can be seen as an additional zero-point in \Cref{fig:AllDelayComparison-ROI-bitrate-Paul}. However, the delay curves for sample content \textit{Johnny} for both approaches of ROI-based encoding shown in  \Cref{fig:AllDelayComparison-offset-Johnny,fig:AllDelayComparison-ROI-bitrate-Johnny} have very low deviation from delay curve for conventional encoding. This is due to the comparatively low temporal complexity.
...
\begin{itemize}
	...
	\item This approach can guarantee minimum quality for the background region by having sufficient bits allocated to the non-ROI to avoid extreme blockiness.
	\item The variable importance of ROI can be factored easily by altering the bias factor $(k)$ to vary the magnitude of bit movement from non-ROI to ROI.
\end{itemize}

\subsection*{Disadvantages}
\begin{itemize}
	\item The delay plots in \Cref{fig:AllDelayComparison} show that there is a higher deviation in bit-consumption behavior with ROI-based bit-allocation scheme compared to ROI-encoding using QP offsets. This might lead to an increased number of dropped frames in extreme conditions.
	\item This approach requires major modifications to the bit-allocation module and hence more complex to implement.
	\item The determination of relative complexity factor $(C_R)$ needs trained models to predict bit-consumption of ROI and non-ROI parts during conventional encoding without using ROI information. This prediction model has bad accuracy for macroblocks encoded with inter-prediction.
	\item The cost-bits model developed to estimate the relative bit-consumption factor ($B_R$) depends on the approach used to compute RDO cost. This varies across different video encoders making it necessary to develop different models for different encoders. 
\end{itemize}
