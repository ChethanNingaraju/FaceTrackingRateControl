...
The face detection module is independent of the encoder and ROI-based encoding schemes discussed in this work. The face detection module uses input video stream to the encoder and marks the ROI at the macro block level. Figure \ref{fig:PaulDefault120FaceMap} shows the face map generated for the frame shown in Figure \ref{fig:PaulDefault120}. Each byte value in the face-map generated by the face detection module represents a macroblock scanned in raster scan order. A value of 0xff signifies macroblock being part of the face or ROI and 0x00 represents a non-ROI macroblock.  The region in white is considered as ROI. This information is used by the bitrate control module of the encoder for ROI-based encoding.
...
The technique described above will avoid the wastage of computation resource by avoiding face detection or tracking when face region is highly static. This will also reduce the error due to face detection with temporal subsampling since face detection is performed immediately after considerable movement of face region is detected. \Cref{fig:FD_all} shows the same frame shown in \Cref{fig:FDnFrameWorstError} with motion vector based variable interval face detection (shown as blue bounding box). It is clear that the inaccuracy in face detection is much lower compared to the approach where face is detected in every 15th frame ($n=15$). 
...
\begin{itemize}
	...
	\item This approach is based on the reuse of motion vectors computed during video encoding. Therefore it has a very low computation complexity.
\end{itemize}
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c|c| }
		...
		\hline 
	\end{tabular}
	\caption{PSNR Comparison for QP offset based ROI encoding}
	\label{Table:QPOffsetPsnr}
\end{table}

The PSNR of ROI-based encoding with QP offset is tabulated in Table \ref{Table:QPOffsetPsnr}. There is a considerable improvement in the PSNR of ROI. There is a corresponding drop in the PSNR of non-ROI which is also reflected in a reduced overall frame PSNR. The drop is not significant leading to an overall increased perceptual quality.
...
The algorithm implemented to adapt QP offset uses the relative area of ROI. The QP offset is calculated using the linear relationship described in (\ref{Eq:QP_offset}). This is a heuristic approximation which was found to yield best perceptual quality across many video contents. The scaled QP offset is then clipped to a value of -6 to avoid large QP offsets which can lead to side-effects discussed in the previous section.
\begin{equation}
\label{Eq:QP_offset}
\begin{aligned}
...
\begin{itemize}
	...
	\item The deviation control mechanism described in section \ref{sec: Bitrate overview: Bitrate deviation} is not in sync with the QP offsets. The deviation control mechanism will increase the QP even for ROI blocks when there is an increased deviation due to over-consumption of bits by ROI macroblocks.
\end{itemize}
