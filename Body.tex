...
%
% 
\chapter{ROI-based Intelligent Bitrate Control} \label{chapter:ROI-RC}
\thispagestyle{empty}% no page number in chapter title page
...
The bitrate control module studied in section \ref{chapter:used-bitrate-control-overview} computes a deviation factor ($D_m^{n'}$) based on accumulated error in frame level bit-consumption after encoding every frame as shown in (\ref{Eq:Frame level bit error accumulation}). In steady-state, the deviation factor ($D_m^{n'}$) takes a value such that error in frame level bit-consumption is minimum. 
