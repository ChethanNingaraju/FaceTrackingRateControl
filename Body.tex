...
\begin{figure}[!h]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Chet_425.png}
		\caption{}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
...
\begin{figure}[!h]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Chet250kbpsDefault.png}
		\caption{}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Chet250kbpsDefaultQuant.png}
		\caption{}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{New_dec/Default/Chet250kbpsDefaultPsnrAbs.png}
		\caption{}
		\label{}
	\end{subfigure}
	\begin{subfigure}[t]{0.49\textwidth}
...
The snapshot of sample input encoded  with QP offset of -8 shown in Figure \ref{fig:QP offset compare 8} has sharper face region compared to same fame encoded with QP offset of -4 shown in Figure \ref{fig:QP Offset image comparison 2}. This is expected since the face region is coded with lower QP due to a larger negative QP offset. The blockiness in the non-ROI, specifically around the arm region has increased. The increase in sharpness in face region masks this blockiness to some extent. However, with further increase in the magnitude of QP offset the blockiness in the background becomes more prominent. The image in Figure \ref{fig:QP offset compare 12} is encoded with QP offset of -12. At this point, the increase in sharpness in the face region is masked by extreme blockiness in the non-ROI, specifically around the arm region. It is clear from the images in Figure \ref{fig:QP offset tuning}. The corresponding PSNR plots show extreme low PSNR in the non-ROI with increasing QP offsets. Therefore, a very high QP offset can have adverse effect the perceptual quality.
...
The data in Table \ref{Table:Mode Specific data} shows the probability of choosing a given prediction mode.  It is clear that skip mode in the most commonly used prediction mode for the encoder configuration discussed in section \ref{sec:Encoder configuration}. This is due to the low target bitrate and mostly static background in video conferencing videos. Therefore, it is very important to have good bits vs cost prediction model for skip macroblocks to compute $B_R$ with reasonable accuracy.
...
\begin{itemize}
	...
	\item Get the number of intra macroblocks, inter-macroblocks and skip macroblocks in the ROI and non-ROI region separately. This can be done using data from the previous frame.
	\item Compute relative complexity of the ROI and non-ROI parts by accumulating costs of macroblocks in these regions.
	\item The relative complexity ($C_R$) is translated to relative bit-consumption ($B_R$) between ROI and non-ROI using  prediction models shown in Figure \ref{fig: Cost vs bits mode}. A pre-computed value is used for the skip macroblocks to account for bits consumed by skip macroblocks.
\end{itemize}
...
The bitrate control module studied in section \ref{chapter:used-bitrate-control-overview} computes a deviation factor ($D_m^{n'}$) based on accumulated error in frame level bit-consumption after encoding every frame as shown in (\ref{Eq:Frame level bit error accumulation}). In steady-state, the deviation factor ($D_m^{n'}$) takes a value such that the error in frame level bit-consumption is minimum. 
...
\begin{table} [h!]
	...
	\begin{tabular}{ |c|c|c|c| }
		...
		\hline 
		Conventional Encoding & Paul640x480, 250kbps & 39.15 & 37.72 \\ 
		& Johnny1280x720 350kbps & 37.93 & 37.37 \\ 
		& Chet640x480 250kbps & 31.35 & 32.91 \\ 
		\hline
		Improved QP Offset based  & Paul640x480, 250kbps & 38.29 & 40.10 \\ 
		ROI encoding & Johnny1280x720 350kbps & 37.50 & 40.74 \\ 
		&  Chet640x480 250kbps & 31.08 & 34.89 \\ 
		\hline
		ROI-based Bit-allocation & Paul640x480, 250kbps & 38.22 & 40.95 \\ 
		& Johnny1280x720 350kbps & 36.94 & 38.57 \\  
		&  Chet640x480 250kbps & 30.55 & 36.17 \\
		\hline 
	\end{tabular}
