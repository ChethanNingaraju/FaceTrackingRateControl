...
The simplest and straight-forward way of creating a bias in the quality between ROI and non-ROI is by using a QP offset for macroblocks belonging to the ROI. A negative QP offset is added to QP allocated by the bitrate control module for ROI macroblocks. The QP offset is used outside of bitrate control module, hence this requires minimum or no modifications inside the bitrate control module. The rate control module obtains feedback from the encoder regarding bit-consumption at every macroblock level\ref{}. This feedback is used by the bitrate control module to modify the rate control QP allocation according to the QP offset added externally to create quality bias for ROI. The bitrate control reacts to usage of reduced QP for ROI macroblcoks by increasing the QP of non-ROI to compensate for the additional bits used by ROI macroblocks. The feedback from the encoder to bitrate control module will ensure that final bitrate is still met. 
